...
def run(output_dir, gt_dir, load_npz=False):
    if load_npz:
        ...
        for subfolder_i in range(len(subfolders)):
            ...
            files_ori = os.listdir(os.path.join(output_dir, subfolder, pred_file_name))
            # subsample 10th
            files = []
            for fo in files_ori:
                # if int(fo[:-4]) %10==1:
                #     files.append(fo)
                files.append(fo)
            # print(output_dir, subfolder, pred_file_name)
            ...
            assert len(set(files_nopng)) == len(files)
            for i in tqdm(range(len(files))):
                ...
                try:
                    gt_2d_key = fa_2d.get_landmarks(gt_for_key)[0][np.newaxis, :, :]
                except:
                    print('not detected face in Ground Truth, use lmk in expression instead')
    ...
    if count > 0:
        for i in keypoint_l:
            if i == None:
                ...
                print('too large, equal to not detected.')
                count += 1
            else:
                new_key_error.append(i)
        keypoint_l = new_key_error
    print(count)
    for l in (keypoint_l):
        if l > 20:
            print(l)    
