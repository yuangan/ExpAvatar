...
class Adapter(nn.Module):
    def __init__(self, channels=[128, 128, 128, 256, 256, 512], nums_rb=1, cin=3, ksize=3, sk=True, use_conv=True):
        ...
        for i in range(len(channels)):
            for j in range(nums_rb):
                if (i != 0) and (j == 0):
                    self.body.append(nn.Conv2d(channels[i - 1], channels[i], kernel_size=3, padding=1))
                    self.body.append(nn.Conv2d(channels[i], channels[i], kernel_size=3, padding=1, stride=2))
                    ...
                    self.body.append(nn.Conv2d(channels[i - 1], channels[i], kernel_size=3, padding=1))
                    self.body.append(nn.Conv2d(channels[i], channels[i], kernel_size=3, padding=1, stride=1))
        ...
        self.model_channels = channels[0]
        self.time_embed = nn.Sequential(
    ...
    def forward(self, x, timesteps, detail_cond):
        # extract features
        features = []
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        x = self.conv_in(x)
...
class DiffusionRigInstaConv(nn.Module):
    ...
    def forward(self, x, t, physic_cond, detail_cond=None, x_start=None, insta=None):
        ...
        if insta is not None:
            insta = self.insta_encoder(insta, t, detail_cond)
            # out_insta = self.cond_insta_layers(insta)
            # scale, shift = th.chunk(out_insta, 2, dim=1)
            # detail_cond = detail_cond * (1 + scale) + shift
            detail_cond = {'detail_cond': detail_cond,
...
class ControlNetConditioningEmbedding(nn.Module):
    ...
    def __init__(
        ...
        # self.conv_in2 = nn.Conv2d(block_out_channels[0], block_out_channels[0], kernel_size=3, padding=1)
        ...
        self.model_channels = block_out_channels[0]
        

    def forward(self, conditioning, t, detail_cond):

        # embedding = self.conv_in(conditioning)
        # embedding = F.silu(embedding)
        # embedding = self.conv_in2(embedding)
        # embedding = F.silu(embedding)
        ...
        embedding = self.conv_out(conditioning)

        return embedding


class ControlNetConditioningEmbedding_instaconv1(nn.Module):
    """
    Quoting from https://arxiv.org/abs/2302.05543: "Stable Diffusion uses a pre-processing method similar to VQ-GAN
    [11] to convert the entire dataset of 512 × 512 images into smaller 64 × 64 “latent images” for stabilized
    training. This requires ControlNets to convert image-based conditions to 64 × 64 feature space to match the
...
class ControlNetConditioningEmbedding_instaconv0(nn.Module):
    """
    Quoting from https://arxiv.org/abs/2302.05543: "Stable Diffusion uses a pre-processing method similar to VQ-GAN
    [11] to convert the entire dataset of 512 × 512 images into smaller 64 × 64 “latent images” for stabilized
    training. This requires ControlNets to convert image-based conditions to 64 × 64 feature space to match the
